...
描述
    ...
    司拥有相同的电话号码。
关于输入
    输入的格式是，第一行正整数指定电话号码薄中号码的数量（最多100000）。余下的每行是一个电话号码。每
    个电话号码由数字，大写字母（除了Q和Z）以及连接符组成
关于输出
    对于每个出现重复的号码产生一行输出，输出是号码的标准格式紧跟一个空格然后是它的重复次数。如果存在
    多个重复的号码按照号码的字典升序输出。如果没有重复的号码，输出一行：
    No duplicates.
例子输入
    12
    4873279
